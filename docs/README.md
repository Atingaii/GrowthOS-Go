...
- 当前完成：第 1～29 节，共 29 节；第二阶段 M0 与第三阶段 M1 均已验收。第三阶段已完成 Lottery 领域、两表、仓储、无偏选择、ephemeral API/React、规则所有权与 Redis Strategy 投影。第 25～26 节在 Participation 中依次交付新用户资格、风险准入与固定有序短路链；第 27～29 节在 Lottery 中依次用具体会员分支证明线性 chain 局限、把 branch/default/target 固化为有界不可变 rooted DAG/三表仓储，并对 exact graph revision 实现封闭 typed、有 step/time budget 的确定求值。第四阶段仍在进行中，下一步第 30 节建立 Strategy/Activity 发布绑定；公共访问控制仍按第 31～35 节推进。
- 当前代码：已有可运行的 Gin 产品进程、类型化配置、`slog`、`request_id`、统一错误、`GET /health`、MySQL `GET /ready`、有界 `sqlx`/Redis pool 和独立前向 Migration 命令；`000001` / `000002` 创建 Strategy/Award，`000003` / `000004` / `000005` 创建 routing graph/node/edge，源码 latest 为 5。`StrategyReader` 可按开关被 cache-aside 装饰。Participation 现有 consumer-owned registration/risk ports 与固定资格链。Lottery 现有会员具体 router、schema v1 `StrategyRoutingGraph`、`StrategyRoutingGraphCreator` / `StrategyRoutingGraphReader`、独立 MySQL Repository，以及未装配的 `StrategyRoutingGraphEvaluationService`：它按 exact identity 读图一次，在 worst-depth 准入后只取一次 Clock/会员事实，以封闭 rule code 逐步命中 exact branch，返回不可变完整 path；调用者取消、内部预算、provider failure 和 invalid value 的优先级固定，任何失败都返回 zero decision。graph Repository/evaluator 均未进入 composition root，长期 `growthos_app` 仍只读旧两表且真实拒绝三张 graph 表；隔离 legacy writer 只写旧两表，隔离 graph repository 身份只写新三表。`make lesson28-mysql-acceptance` 已在一次性 MySQL 8.4.11 上验证六组 Integration 与零残留；默认长期 Compose 也已从 v2 原地前向到 v5 并通过 smoke，隔离 Lottery/cache acceptance 已在 v5 重跑通过且未改变长期资源 identity。
- 当前架构承诺：Go 优先、渐进式演进、数据库按需求迁移。
- 当前明确未做：生产注册/风险/会员 fact adapter、在线资格/会员路由/graph evaluation 门控、graph 发布/Activity 绑定、graph/evaluator composition-root、graph 公开 API/UI、正式 Lottery Draw/Result、登录认证、RBAC/对象级授权、前端权限投影、浏览器端到端验收、幂等、库存、发奖、Strategy 更新/精准失效、MQ、微服务和 Java 实现。React `/lottery` 只展示不可恢复的 ephemeral Award 选择；其余工作台业务仍为明确 Mock/本地交互。Redis 不缓存 graph、资格、会员事实、route decision、权限、库存、随机选择或最终结果；M1 本机证据、第 28 节结构/仓储验收与第 29 节内核测试都不是业务 SLO。INV-03 仍无最终结果事实可供验证。

...
- 当前完成：第 1～30 节，共 30 节。第 30 节在 exact graph evaluator 之后补上 Lottery create-only Strategy snapshot，以及 Marketing-owned Activity draft / immutable publication version / rollback / retire / resolve gate；普通发布和回滚都追加版本，状态变更使用 `state_version` CAS，跨上下文由 Lottery ACL 精确复核 graph 与 terminal Strategy revision 闭合集。下一节第 31 节开始公共访问控制模型与威胁边界，第 32～35 节停止线不变。

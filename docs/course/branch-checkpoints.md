...
第 27 节从第 26 节最终检查点 `47fc94d` 线性创建。`2d7728a` 与 `57a3216` 先把 premium override、standard baseline default、unknown 失败关闭、事实/决定所有权和第 28～35 节停止线冻结为新增产品决定；`076e399` 只实现 Lottery 会员事实、具体 policy、纯 Route 与一跳 path；`b307f1a` 根据交叉审查把稳定 branch literal 与显式 tier switch 变成可执行契约；`42caed9` 增加 consumer-owned reader、单一 Clock/freshness、取消优先和安全 Cause；`2dc49b1` 守住 Lottery/Participation 与无通用 Engine 的边界；`8ebc94a` 再校验 decision/branch/reason/path 内部一致；`544f4af` 诚实校准 revision 字符串尚未绑定唯一内容。课程、API、31 道问答、设计手记、架构当前态和 QA 计划依次由 `a6d2d49`、`5ecb061`、`f2e5e07`、`d57b963`、`5460bde`、`331c8c7` 交付；`6db36dd`、`59499fb`、`a04d89d` 再根据终审校准 revision 确定性、递归扫描泛型函数并对齐停止线证据。全局索引与 clean-worktree 门禁仍以同名远端冻结 tip 为准。本节没有 membership adapter、DB/schema、Redis、HTTP/React、Strategy load/Selector、Activity、权限或浏览器 E2E。

## 稳定章节分支与累计快照

课程长期保留稳定章节分支与累计快照；本节技术与文档门禁已经通过，但最终 QA/台账还需要形成一次冻结提交并推送，因此 Git 状态与验收事实分开记录：

| 类型 | 分支规则 | 是否移动 | 当前事实 |
| --- | --- | --- | --- |
| 单节稳定分支 | `codex/lesson-XX-...` | 一节验收结束后保持稳定 | 第 27 节在“包含最终 QA/台账的冻结提交”推送成功后，以同名远端实际 tip 冻结；最终 SHA 由 Git 产生，不在提交前猜测 |
| 本次待冻结分支 | `codex/lesson-27-responsibility-chain-limits` | 只再接受配套文档、索引和最终证据提交 | 实现与主要正文已推送；根代理复跑最终门禁、提交 QA/台账并验收 push 后完成冻结 |
| 最新累计快照 | `codex/complete-implementation` | 每节验收后快进 | 当前为第 26 节最终检查点 `47fc94d`；第 27 节冻结后再 fast-forward 到同一远端 tip |

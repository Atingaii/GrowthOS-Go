...
| 类型 | 分支规则 | 是否移动 | 当前事实 |
| --- | --- | --- | --- |
| 单节稳定分支 | `codex/lesson-XX-...` | 一节验收结束后保持稳定 | 第 21 节实现、协议/隔离验收与最终文档均推送后，冻结同名远端 tip |
| 最新累计快照 | `codex/complete-implementation` | 每节验收后快进 | 第 21 节最终检查点和门禁通过后快进至该节稳定分支 tip |

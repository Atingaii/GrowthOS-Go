...
共享工作台让“不同人员看到不同界面”的需求变得可见，但当前 `activeRole`、Mock 用户、工作区入口和页面隐藏都不是真实权限系统。继续向真实业务接入前，必须先停止堆叠页面级 `if`，在第 23 节单独回答访问控制的基础问题：

- 谁是可信主体，角色、权限、资源、动作和数据范围分别表达什么；
- 用户、运营、MCP、Agent、审批与审计职责怎样遵守最小权限和职责分离；
- 浏览器、同源网关、Go API、MySQL 与外部 Tool 的信任边界在哪里；
- 未登录、权限不足、对象不存在和跨范围访问分别怎样失败关闭；
- 为什么前端裁剪导航只能改善体验，不能替代服务端逐请求授权；
- 哪些威胁必须在真实会话与服务端 RBAC 出现前先写成可验证约束。

第 23 节只建立权限语言、角色—权限矩阵、信任边界和威胁模型，不提前宣称真实登录、会话、服务端强制执行或前端权限裁剪已经实现。后续课程会在第 23 节分支中正式按连续编号修订；在这些边界落地前，本页继续保持 development/test、显式 StrategyID、无自动重试和非持久化候选语义。

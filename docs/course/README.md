...
- 当前真实 Lottery API 及其 React 消费者仍只产生并展示不持久化的 ephemeral selection；`reward` 只是奖励候选，`no_reward` 是正常候选结果。Redis 不缓存资格、权限、会员事实、路由决定、随机选择或 Draw/Result，也不进入 API readiness。第 25～26 节的固定 Participation 资格链、第 27 节的 Lottery 会员路由与第 28 节的持久化路由图仍是彼此独立、未装配的内核；三张图表存在不等于图已执行、已发布或已获得管理 API。正式 Draw/Result、认证、对象级授权、幂等、Activity/次数等完整资格与 Lottery 前置门控、库存、积分扣减和发奖均未实现；INV-03 仍未满足。
...
- 下一节是第 29 节“实现规则决策引擎”：只执行第 28 节已经严格验证和恢复的有界图，不把持久化存在误写为已发布或可执行。第 31～35 节仍按“公共模型 → 会话 → 服务端强制 → 前端感知 → 越权验收”演进，并在第 36 节首个真实运营后台复用。
...
数据库同样不是预制终态。第 13 节先建立连接、账号隔离、readiness 与前向 Migration 机制；第 18 节以 `000001` / `000002` 分别创建 `lottery_strategy`、`lottery_strategy_award`；第 28 节再以 `000003`～`000005` 依次创建 graph、node、edge 三张表，当前 latest 为 5。每个版本仍只包含一条 MySQL DDL，使 dirty version 能精确定位；长期 API 身份仍只保留既有业务表权限，图仓储使用隔离测试身份验证精确三表 `SELECT, INSERT`，且 adapter 尚未装配进运行时。后续每一次字段、索引、新表、冗余快照或宽表变化，必须记录新业务需求、现有设计的不足、Migration、查询/并发影响和 QA 证据。已执行的 Migration 只追加，不回写历史。

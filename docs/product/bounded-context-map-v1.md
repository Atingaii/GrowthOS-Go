...
积分、优惠券和参与次数的实现会逐步出现。当前先共享“发放、持有、使用、补偿”的语言；合规、团队或流量差异扩大后，再评估子域或上下文拆分。
...
第 7 节需要为主链路、分析链路和 AI 操作分别定义 QPS、P99、可用性、一致性、幂等、审计、追踪和恢复目标。第 8 节再基于本地图和 NFR 绘制 V0 系统设计，不提前画最终部署图或 ER 图。

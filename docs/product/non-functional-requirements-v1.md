...
| 项目 | 当前事实 |
| --- | --- |
| Go 运行时基线 | 第 11～15 节已验收 Gin 进程、`GET /health`、MySQL `GET /ready`、类型化配置、结构化日志、请求关联与统一错误；错误响应使用 `Cache-Control: no-store`；尚无业务 API，也没有性能或可用性实测 |
| 业务数据库 | MySQL 8.4 连接池、账号隔离和前向 Migration 机制已通过功能联调；没有业务表，第 18 节才首次建表 |
| React | 第 14 节框架与第 15 节系统状态页已完成；系统探针通过 Vite 同源代理真实读取 Go API，其余业务页面仍使用 Mock；要求 Node.js `>=22.22.2`、pnpm `10.13.1` |
...
第 13 节真实 MySQL 8.4.11 集成测试证明身份、连接、探针和 Migration 机制能工作；第 15 节真实浏览器关联验收证明系统状态页能区分正常、数据库不可用和 API 离线，并能展示对应请求关联信息。两者都是有界功能证据，不是连接池容量、吞吐、持续可用性、灾备或业务性能实测。因此本版本仍只能评审目标是否完整、可测量和符合业务风险，不能证明系统已达到候选 SLO。

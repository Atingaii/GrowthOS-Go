...
**状态：** v1 目标基线；M0 工程探针与 M1 Strategy 缓存本地基线已实测；第 25～27 节资格/会员路由完成内核验证，第 28 节完成 routing graph 结构与隔离 MySQL 仓储验收，第 29 节完成未发布/未装配的 closed graph evaluator 与覆盖/race/fuzz 验证，业务 SLO 尚未实测

**更新日期：** 2026-08-30

**来源章节：** [第 7 节：确定非功能需求](../course/part-01/lesson-07-non-functional-requirements.md)；第 23 节以[规则需求与所有权边界](../course/part-03/lesson-23-lottery-strategy-rule-requirements.md)补充未来决策正确性约束；第 24 节以[首次 Redis Strategy 缓存](../course/part-03/lesson-24-redis-strategy-cache.md)登记 M1 本地证据；第 25～26 节以[首个 Participation 资格切片](../course/part-04/lesson-25-user-eligibility.md)和[固定前置资格链](../course/part-04/lesson-26-responsibility-chain.md)登记资格决定证据；第 27 节以[具体会员路由](../course/part-04/lesson-27-responsibility-chain-limits.md)登记显式分支/default/path；第 28 节以[Strategy Routing Graph 持久化](../course/part-04/lesson-28-rule-tree-schema.md)登记拓扑资源上限、latest 5、仓储快照和隔离权限证据；第 29 节以[封闭 Strategy 路由图求值](../course/part-04/lesson-29-rule-decision-engine.md)登记 exact snapshot、worst-depth/step/time/cancel budget、zero-decision 与求值内核质量证据
...
| 业务数据库 | MySQL 8.4 连接池、账号隔离和前向 Migration 已通过功能联调；`000001` / `000002` 创建 Strategy/Award，`000003` / `000004` / `000005` 创建 routing graph/node/edge，源码 latest 5。Strategy 与 graph Repository 都用手写 SQL、写事务和只读 RR 快照。长期 runtime 仍只有旧两表 `SELECT` 且拒绝 graph 三表；legacy writer 只在旧两表拥有测试 `SELECT, INSERT`，graph repository 身份只在新三表拥有测试 `SELECT, INSERT` |
| Lottery 领域 | Strategy/Award、Repository、WeightedSelector 与 CryptoSource 已覆盖不变量、快照、错误和边界；第 21 节形成 ephemeral HTTP 链，第 24 节缓存 Strategy 投影。第 27 节交付 concrete 会员 router；第 28 节交付 `(GraphID, Revision)` create-only schema v1 rooted DAG、128 nodes/256 edges/16 depth 上限、严格恢复、窄端口和未装配 graph MySQL adapter；第 29 节交付未装配的 closed typed evaluator，以一次 exact graph/一次 Clock/一次 fact 形成有界单路径 decision。仍没有 Activity/发布绑定、runtime/API 求值链、认证、资格门控、Draw/Result、幂等、库存、发奖或业务 SLO 实测 |
...
| 规则决策质量 | 第 25～26 节验证具体资格、固定顺序、shared as-of、短路和最小 trace；第 27 节验证 confirmed premium/standard 的显式分支/default/path，unknown 与技术失败不落默认边；第 28 节验证 graph 的唯一 root、全可达、无环、terminal、exact branch/default 与 128/256/16 资源边界。第 29 节再验证 exact identity、封闭 operator、确定 single path、ordered defensive-copy evidence、`Depth() <= maxSteps` 准入、actual step hard stop、positive child deadline、caller/internal/provider 优先级与所有失败 zero decision。授权拒绝、Activity/次数等完整资格、持久化审计及与 `no_reward` 的 runtime 编排仍待后续章节验证 |
| 求值内核质量门 | 第 29 节当前代码候选上，Lottery domain/application atomic coverage 分别为 93.6%/88.3%、合并 92.1%；全仓普通与 `-race` 测试通过；独立 10 秒 evaluator fuzz 通过 2,569,203 execs，新发现 1 个 interesting input（总数 41）。这些数据只是内核回归证据，不是业务覆盖率、生产并发安全、穷举证明或 SLO |
...
| 安全与故障演练 | 已验证 MySQL 长期运行身份仅旧两表 `SELECT`，graph SELECT 真实 1142；第 28 节 disposable MySQL 8.4.11 以相互隔离身份验证 legacy writer 只写旧两表、graph writer 只写新三表，并核对零残留/长期 Docker 快照不变；第 29 节候选又重跑同一 MySQL 8.4.11 六组 Integration 并通过，只作为 schema/Repository 上游回归。长期 Compose 原地 `2:0 -> 5:0`、smoke 与隔离 v5 Lottery/cache acceptance 均通过，旧表指纹和长期资源 identity 不变。Redis ACL、唯一回环端口、internal cache 网络、非 root/只读/capability、受限 Host/framing/size 和 502/504 关联也已覆盖。这些都不等于 evaluator 已装配、认证、对象授权或生产渗透测试 |

第 13～24 节证据依次证明 MySQL/Migration、浏览器探针、Compose M0、Lottery 聚合/Schema/Repository/selector、受限 HTTP/React 链和 Strategy cache M1；第 25～27 节只以 domain/application 单元、fuzz、并发、取消和架构测试验证资格链与 concrete 会员路由。第 28 节新增结构/仓储证据而非业务负载：一次性 MySQL 8.4.11 中六组 Integration 全通过，覆盖 latest 5、三表约束、两类隔离 writer、graph round-trip/RR/回滚/边界/EXPLAIN，任务资源零残留且长期 `growthos` Docker 快照不变。第 29 节在该 validated aggregate 之上实现内部 exact graph evaluator，以普通/race/fuzz/覆盖率/架构测试验证遍历与失败边界，并重跑第 28 节 MySQL 六组上游回归。它们都没有装配 API、增加 UI 或验证 Activity/权限；求值 `maxDuration` 是 cooperative safety budget，不是决策延迟或业务可用性 SLO。M1 的每组 500 请求、50 RPS、10 秒、最多 16 workers 也仍只是当前本机开发证据；没有正式 Draw 持久化、资格/库存争用、长稳、多主机或生产数据分布。

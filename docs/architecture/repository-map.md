...
第 9 节已验收仓库工程基线。第 11 节已把 HTTP 占位边界落实为最小 Gin 进程、健康接口和可控生命周期，并留下自动化与真实进程 QA 证据。其他占位目录不代表对应产品能力已经实现。
...
| `internal/infrastructure/httpapi` | Gin router、`GET /health` handler 与 HTTP 契约测试 | 第 11 节 |
| `internal/infrastructure/httpserver` | 标准库 HTTP Server 运行、context 取消、5 秒优雅关闭与错误传播 | 第 11 节 |
| `internal/*` | 预留私有领域与基础设施边界 | 随对应领域章节引入 |
| `pkg` | 可被外部导入的少量稳定 Go 包 | 仅在确有跨模块公共契约时 |
| `configs` | 无秘密、可版本化的配置示例 | 第 12 节起 |
...
- 监听地址、关闭预算的配置化，以及结构化日志与统一错误码留到第 12 节；第 11 节默认监听 `:8080`、关闭窗口为 5 秒。

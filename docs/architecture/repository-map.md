...
| `internal/platform/appconfig` | `GROWTHOS_` API/Migration 独立配置、直接/文件秘密二选一、公开默认值、秘密脱敏、Lottery feature gate 与跨组件 timeout 校验 | 第 12～13、16、21 节 |
...
| `scripts/compose-smoke.sh` | 四常驻/两 one-shot 状态、latest 2、两表 SELECT-only 精确授权及负向 DML、探针/ephemeral 404、HTTP 契约与宿主机端口隔离的只读冒烟检查 | 第 16、18～19、21 节 |
| `scripts/compose-lottery-api-acceptance.sh` | 以随机 project/secret/volume/image 创建一次性真实纵向环境，验证 Lottery success/failure、64 个请求且最大并行 16、网关故障、数据指纹与所有权校验清理 | 第 21 节 |
| `deploy/compose` | Web/API/MySQL/Redis 四个常驻服务，Migrate/mysql-grants 两个 one-shot，三张隔离网络、MySQL data/socket named volume、文件秘密、ephemeral feature 配置和仅回环 Web 端口 | 第 16、18～19、21 节 |
...
| `deploy/docker` | API/Migrator/Web/Redis 构建边界、非 root 运行入口，以及限制 Host/framing/size/timeout/request ID 的 Nginx 同源网关 | 第 16、21 节 |
...
| `docs` | 产品、架构、决策、QA、第一性原理设计推导、面试问答和课程事实；第 23 节增加 Lottery 规则需求基线与 ADR-0019 | 全程 |
...
- 第 19 节已建立 Lottery Strategy 窄仓储，第 20 节已建立无偏加权 Award 选择，第 21 节已建立两表 `SELECT` 运行权限和受限 ephemeral API，第 22 节已交付真实 ephemeral React 页面，第 23 节已建立规则事实所有权与演进停止线。第 24 节只补充可重建 Strategy 读取投影的 Redis 缓存；正式资格、Draw/Result、幂等、库存与发奖必须由后续真实问题驱动，写权限不会因“常见 CRUD”被提前加入。

...
| `web` | 统一 React 用户端、运营端、MCP 与 AI Operator 框架；系统状态真实联调，`/lottery` 仍使用 `Math.random()` Mock，其余业务页面仍为 Mock | 第 14～15 节 |
...
- React、TypeScript、Vite、Tailwind CSS、Lucide、Recharts 和 Zustand 在第 14 节接入；第 15 节只接通系统探针。第 21 节后端 Lottery API 已存在，但 React 尚无 lottery API adapter，`/lottery` 继续使用 `Math.random()` Mock，等待第 22 节真实联调。
- 第 16 节已引入 Compose 本地开发环境，但它仍是单机开发拓扑：没有镜像 digest 固定、内部 TLS、生产资源配额、Secret Manager 或业务 Redis 接入。
- 第 19 节已建立 Lottery Strategy 窄仓储，第 20 节已建立无偏加权 Award 选择，第 21 节已建立两表 `SELECT` 运行权限和受限 ephemeral API。真实抽奖页、规则和 Redis 分别等待第 22～24 节；正式 Draw/Result、认证、幂等、库存与发奖则必须由后续真实问题驱动，写权限不会因“常见 CRUD”被提前加入。

...
import { ArrowRight as ArrowRightIcon, TrendingUp } from "lucide-react";
...
export const GrowthOSLogo: React.FC<{ className?: string; iconOnly?: boolean }> = ({
  className = "h-8",
  iconOnly = false,
}) => {
  return (
...
const FunnelTooltipContent = ({
  ...
  active?: boolean;
  payload?: { payload: (typeof funnelData)[0] }[];
}) => {
  ...
  return (
    <div className="bg-white dark:bg-neutral-900 border border-stone-200 dark:border-neutral-700 px-3 py-2 text-xs shadow-sm">
      ...
      <p className="text-stone-500 dark:text-stone-400 font-mono">
        {d.label} 人{d.rate ? `  ·  转化 ${d.rate}` : ""}
      </p>
    </div>
  );
};
...
export const GrowthFunnelIllustration: React.FC<{ className?: string }> = () => {
  return (
    <div className="w-full">
      {/* Stage stats row */}
      <div className="mb-4 grid grid-cols-1 gap-px bg-stone-100 sm:grid-cols-3 dark:bg-neutral-800">
        {funnelData.map((d, i) => (
          <div key={d.name} className="bg-white dark:bg-[#141414] px-4 py-3">
            ...
            <p className="text-lg font-bold tabular-nums" style={{ color: FUNNEL_COLORS[i] }}>
              {d.label}
            </p>
            {d.rate && (
...
const ArchitectureArrow: React.FC<{ color: string }> = ({ color }) => (
  <div className="flex shrink-0 rotate-90 items-center justify-center px-1 sm:rotate-0">
    <ArrowRightIcon className="h-5 w-5" style={{ color }} aria-hidden="true" />
  </div>
);
      ...
      {nodes.map((node, i) => (
        <React.Fragment key={node.label}>
          ...
          >
            ...
            <div className="w-2 h-2 rounded-full mt-3" style={{ background: node.dotColor }} />
          </div>
          {i < nodes.length - 1 && <ArchitectureArrow color={arrowColors[i]} />}
        </React.Fragment>
      ))}
    </div>
  );

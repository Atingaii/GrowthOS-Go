...
func TestDefaultUsesOnlyPublicNonSecretValues(t *testing.T) {
	...
	if config.GovernanceMySQL.Password != "" {
		t.Fatal("Default() unexpectedly contains a Governance password")
	}
}
...
func TestLoadAppliesCompleteOverride(t *testing.T) {
	variables := map[string]string{
		...
		governanceMySQLUserVariable:            "governance-user@private",
		governanceMySQLPasswordVariable:        " arbitrary Governance password \x00 ",
		governanceMySQLReadTimeoutVariable:     "21s",
		governanceMySQLPingTimeoutVariable:     "6s",
		governanceMySQLMaxOpenConnsVariable:    "18",
		governanceMySQLMaxIdleConnsVariable:    "10",
		governanceMySQLConnMaxLifetimeVariable: "9m",
		governanceMySQLConnMaxIdleTimeVariable: "5m",
	...
	want := Config{
		...
		GovernanceMySQL: MySQLConfig{
			...
			User:                  "governance-user@private",
			Password:              " arbitrary Governance password \x00 ",
			PingTimeout:           6 * time.Second,
			MaxOpenConnections:    18,
			MaxIdleConnections:    10,
			ConnectionMaxLifetime: 9 * time.Minute,
			ConnectionMaxIdleTime: 5 * time.Minute,
		},
		Identity: IdentityConfig{
...
func TestLoadRequiresReadinessBudgetBeforeHTTPWriteDeadline(t *testing.T) {
	...
	config, err := Load(mapLookup(apiVariables(map[string]string{
		...
		governanceMySQLPingTimeoutVariable: "2s",
...
func TestPasswordFilesLoadForAPIAndMigration(t *testing.T) {
	...
	loaders := []loader{
		...
		{
			name:                  "governance",
			directVariable:        governanceMySQLPasswordVariable,
			fileVariable:          governanceMySQLPasswordFileVariable,
			companionPasswordName: mysqlPasswordVariable,
			loadPassword: func(lookup LookupFunc) (string, error) {
				config, err := Load(lookup)
				return config.GovernanceMySQL.Password, err
			},
		},
		{
			name:           "migration",
...
func TestSecretBearingConfigsRedactCommonFormattingBoundaries(t *testing.T) {
	...
	values := []struct {
		...
		{name: "governance root", value: Config{GovernanceMySQL: MySQLConfig{Password: secret}}},
...
func apiVariables(overrides map[string]string) map[string]string {
	...
	values := map[string]string{
		...
		governanceMySQLPasswordVariable: "test-governance-password",

...
func TestLesson27MembershipRoutingKeepsContextOwnershipAndEngineStopLine(t *testing.T) {
	...
	for _, checkedPackage := range packages {
		t.Run(checkedPackage.name, func(t *testing.T) {
			...
			if err != nil {
				t.Fatalf("read %s: %v", checkedPackage.directory, err)
			}
			...
			for _, entry := range entries {
				...
				for _, imported := range file.Imports {
					...
					if strings.HasPrefix(importPath, lesson27ModuleImportPrefix) &&
						importPath != checkedPackage.allowedProjectImport {
				...
				for _, declaration := range file.Decls {
					...
					for _, specification := range general.Specs {
						...
						if _, forbidden := forbiddenTypes[typeSpecification.Name.Name]; forbidden {
							t.Errorf("%s prematurely declares generic routing type %s", path, typeSpecification.Name.Name)
						}
					}
				}
